...
export function XXNetwork({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        xxdk.InitXXDK().then(async(xx: XXDKUtils) => {
            ...
            // ⚠️ CLIENT 2: Using different statePath to avoid conflicts with client 1
            const statePath = "xx_client2"
            ...
            const stateExists = localStorage.getItem('cMixInitialized_client2');
            if (stateExists === null || !stateExists) {
                await xx.NewCmix(ndf, statePath, secret, "")
                localStorage.setItem('cMixInitialized_client2', 'true');
            }
...
export function XXDirectMessages({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        // ⚠️ CLIENT 2: Using different localStorage key
        var dmIDStr = localStorage.getItem("MyDMID_client2");
        if (dmIDStr === null) {
            console.log("🔵 CLIENT 2: Generating DM Identity...");
            ...
            localStorage.setItem("MyDMID_client2", dmIDStr);
        }
        console.log("🔵 CLIENT 2 - Exported Codename Blob: " + dmIDStr);
        ...
        xxdk.dmIndexedDbWorkerPath()
            .then((workerPath: string) => {
                ...
                console.log("🔵 CLIENT 2 - ✅ DM Worker Path: " + workerPath.toString());
            ...
            xx.NewDMClientWithIndexedDb(xxNet.GetID(), notifications.GetID(),
                ...
                .then((client) => {
                    // 🆔 CLIENT 2 IDENTITY: Logs THIS client's token and public key
                    console.log("🔵 CLIENT 2 - ✅ DMTOKEN: " + client.GetToken());      // ← This client's token
                    console.log("🔵 CLIENT 2 - ✅ DMPUBKEY: " + Buffer.from(client.GetPublicKey()).toString('base64')); // ← This client's pub key
                    ...
                    xxNet.WaitForNetwork(60000)  // Increased from 30s to 60s for Pi
                        .then(() => {
                            console.log("🔵 CLIENT 2 - ✅ Network is ready");
                        })
                        .catch((err) => {
                            console.error("🔵 CLIENT 2 - ❌ Network wait failed:", err);
                        });

                    // When the network goes healthy, signal that to anything
                    // waiting on the client that it is ready.
                    setDMClient(client);
                    console.log("🔵 CLIENT 2 - ✅ DM Client initialized and ready!");
                })
                .catch((err) => {
                    console.error("🔵 CLIENT 2 - ❌ Failed to initialize DM client:", err);
                });
        })
        .catch((err) => {
            console.error("🔵 CLIENT 2 - ❌ Failed to get DM IndexedDB worker path:", err);
        });
    }, [xx, xxNet]);
...
// XXDMSend - Updated to send to another user (CLIENT 2 VERSION)
export async function XXDMSend(
    ...
    return await dm.SendText(
        ...
        console.log("🔵 CLIENT 2 - ✅ Message sent successfully:", sendReport);
        return true;
    }).catch((err) => {
        console.log("🔵 CLIENT 2 - ❌ Could not send: " + err)
        return false
    });
}

// Component to display this client's credentials (CLIENT 2 VERSION)
export function XXMyCredentials() {
    ...
    return (
        <div className="p-4 bg-purple-900/20 rounded space-y-2">
            <h3 className="text-lg font-bold">🔵 CLIENT 2 - MY CREDENTIALS (Share these with Client 1)</h3>
            <div>
...
export function XXMsgSender() {
    ...
    const handleSubmit = async () => {
        ...
        if (await XXDMSend(dm, msgToSend, recipientPubKey, recipientToken)) {
            setMessage("");
            console.log("🔵 CLIENT 2 - ✅ Message sent to recipient");
        }
    }

    return (
        <div className="space-y-4 p-4 bg-green-900/20 rounded">
            <h3 className="text-lg font-bold">🔵 CLIENT 2 - 📤 SEND MESSAGE TO CLIENT 1</h3>
            
            <div>
                <label className="block mb-1 font-semibold">Recipient's Token (from Client 1):</label>
                <Input 
                    type="text" 
                    placeholder="Paste Client 1's token here..."
            ...
            <div>
                <label className="block mb-1 font-semibold">Recipient's Public Key (from Client 1):</label>
                <Input 
                    type="text" 
                    placeholder="Paste Client 1's public key here..."
